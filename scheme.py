...
def scheme_eval(expr, env, third =None): # Optional third argument is ignored
...
def eval_all(expressions, env):
    ...
    while expressions != nil:
        ...
        evalualted = scheme_eval(expressions.first, env)
...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        while frame != None:
            ...
            frame = frame.parent
        # END PROBLEM 3
        raise SchemeError('unknown identifier: {0}'.format(symbol)) #12 errors here
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        ev_oper = operands.map(lambda operand: scheme_eval(operand, env))
        return scheme_apply(self, ev_oper, env)
        # END PROBLEM 5
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('incorrect amount of parameters')
        # END PROBLEM 4
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        bind = bindings.first
        check_form(bind, 2, 2)
        inputs = Pair(bind.first , inputs)
        vals = Pair(scheme_eval(bind.second.first,env), vals)
        bindings = bindings.second
        if not scheme_symbolp(bind.first):
            raise SchemeError('incorrect symbol')

    return env.make_child_frame(inputs,vals)
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            # BEGIN PROBLEM 20
            opr = scheme_eval(first, env)     # When scheme_optimized_eval receives an expression in a tail context, then it returns an Thunk instance.
            check_procedure(opr)                # Otherwise, it repeatedly evaluates expressions within the body of a while statement, updating result in each iteration.
            result = opr.eval_call(rest, env)   #  All tail calls should call scheme_eval with True as a third argument, indicating a tail call. Think about what Scheme expressions are in a tail context!
            # END PROBLEM 20
    return result
