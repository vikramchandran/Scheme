...
def scheme_read(src):
    ...
    if val == 'nil':
        ...
        # BEGIN PROBLEM 1
        def remove_paran(src):
            if src.current() != '(':
                return read_tail(src)
            else:
                return Pair(scheme_read(src), remove_paran(src))
        if src.current() == '(':
            return Pair(scheme_read(src), remove_paran(src))
        return read_tail(src)
        # END PROBLEM 1
...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            raise SyntaxError('attempted to call nothing')
        elif src.current() == ')':
            # BEGIN PROBLEM 1
            src.remove_front()
            return nil
            ...
            src.remove_front()
            if src.current() == '(':
                eval = scheme_read(src)
                if src.current() == ')':
                    src.remove_front()
                    return eval
                raise SyntaxError('wrong dot form')
            if src.current == ')':
                src.remove_front()
                return nil
            end = src.remove_front()
            if src.current() == ')':
                src.remove_front()
                return end
            raise SyntaxError('wrong dot form')
            ...
            # BEGIN PROBLEM 1
            if src.current() == '(':
                raise SyntaxError('wrong entry contains (')
            first_param = scheme_read(src)
            def remove_paran(src):
                if src.current() != '(':
                    return read_tail(src)
                else:
                    return Pair(scheme_read(src), remove_paran(src))
            if src.current() == '(':
                return Pair(first_param, remove_paran(src))
            return Pair(first_param, read_tail(src))
            # END PROBLEM 1
    except EOFError:
